...
# Read in dataset
stai_data <- read_rds(paste0(redcap_interim_data_loc, "/redcap-stai-data", ".Rds")) %>% 
...
stai_data <- stai_data %>% 
  mutate(
    ...
    stai_baseline_trait_total = rowSums(select(., contains("stai") & contains("baseline"))),
    stai_followup_trait_total = rowSums(select(., contains("stai") & contains("followup")))
  )

# Save data
saveRDS(stai_data, here("data", "interim", "redcap", "step-02", "redcap-stai-data.Rds"))
