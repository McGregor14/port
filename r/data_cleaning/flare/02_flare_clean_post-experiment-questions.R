...
post_experiment_questions <- post_experiment_questions_raw %>%
  mutate(
    did_remove_headphones = if_else(
      !is.na(headphones_removal_point),
      TRUE,
      did_remove_headphones
    ),
    headphones_removal_point = clean_strings(headphones_removal_point),
    headphones_removal_point =
      str_replace_all(headphones_removal_point, "hadn t", "hadnt"),
    across(
      .cols = c(headphones_removal_point, task_environment),
      .fns = ~ fct_infreq(as_factor(.))
    )
  ) %>%
  rename(experiment_unpleasantness_rating = experiment_unpleasant_rating,
         not_alone = was_alone) %>%
