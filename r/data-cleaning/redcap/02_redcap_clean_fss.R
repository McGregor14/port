...
# Read in dataset
fss_data <-
  read_rds(paste0(redcap_interim_data_loc, "/redcap-fss-data", ".Rds")) %>%
...
fss_data <- fss_data %>%
  mutate(
    ...
    fss_baseline_total = rowSums(select(
      ., starts_with("fss") & contains("baseline")
    )),
    fss_followup_total = rowSums(select(
      ., starts_with("fss") & contains("followup")
    )),
    
    # Social Fears (SOC) subscale: 5, 8, 9, 10, 15, 20, 27, 33, 41, 44, 47, 51, 52
    fss_baseline_soc = rowSums(select(
      .,
      c(
        fss_baseline_5,
        fss_followup_8,
    ...
    fss_followup_soc = rowSums(select(
      .,
      c(
        fss_followup_5,
        fss_followup_8,
    ...
    fss_baseline_ago = rowSums(select(
      .,
      c(
        fss_baseline_2,
        fss_baseline_3,
    ...
    fss_followup_ago = rowSums(select(
      .,
      c(
        fss_followup_2,
        fss_followup_3,
    ...
    fss_baseline_inj = rowSums(select(
      .,
      c(
        fss_baseline_1,
        fss_baseline_4,
    ...
    fss_followup_inj = rowSums(select(
      .,
      c(
        fss_followup_1,
        fss_followup_4,
    ...
    fss_baseline_sex = rowSums(select(
      .,
      c(
        fss_baseline_25,
        fss_baseline_26,
    ...
    fss_followup_sex = rowSums(select(
      .,
      c(
        fss_followup_25,
        fss_followup_26,
    ...
    fss_baseline_ani = rowSums(select(
      .,
      c(
        fss_baseline_13,
        fss_baseline_16,
    ...
    fss_followup_ani = rowSums(select(
      .,
      c(
        fss_followup_13,
        fss_followup_16,
...
# Save data
saveRDS(fss_data,
        here("data", "interim", "redcap", "step-02", "fss.Rds"))
