...
redcap_raw <-
  read_csv(
    here(
      ...
      "raw",
      "redcap",
      "2023-10-03_anonymised-dataset_tm.csv"
    ),
    col_names = TRUE,
    trim_ws = TRUE
  )
...
redcap_data <- redcap_data %>%
  ...
  rename_with(
    .fn = ~ gsub(pattern = '(asi|cbas|fss|gad7|ius12|phq9|pmh|pswq|stai|wsas)_(\\d+)_(.*)',
                 replacement = '\\1_\\3_\\2',
                 .x),
  ...
  # Fix "ethic" typo
  rename_with(.fn = ~ str_replace(., "ethic", "ethnic"),
              .cols = matches('^ethic')) %>%
  
  # Timestamp specific renaming
  rename_with(
    .fn = ~ gsub(pattern = '(port2|port3|port5)_(.*)_(survey)_(timestamp)',
                 replacement = '\\2_\\4',
                 .x),
