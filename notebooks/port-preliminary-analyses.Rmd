...
#### Combined
```{r}
secondary_models_tidy1 <- rbind(sec_acq_minus_model_tidy1, sec_ext_plus_model_tidy1) %>% 
  mutate(sig_05 = if_else(p.value < 0.05, TRUE, FALSE),
         sig_05_adj = if_else(pval.adj < 0.05, TRUE, FALSE),
         term = factor(if_else(str_detect(term, "exp_"), "fc", term)),
         iv = case_when(
           ...
           TRUE ~ TRUE),
         term = factor(term, level = c("demographics_biological_sexmale",
                 "demographics_age_at_screening_z",
                 "ieso_treatment_completed_total_z",
                 "gad7_treatment_total_1_z",
                 "fc")))
...
secondary_models_tidy1 %>%
  ...
  labs(
    title = "Secondary analyses: age & sex adjustment",
    subtitle = "\nOutcome: last treatment session GAD-7 scores
    \nModel 1: mean acquisition CS- expectancy
    \nModel 2: mean extinction CS+ expectancy",
    caption = paste0("Intercepts",
