...
ggsave(
  here("reports", "figures", "primary-regression-forest-plot.pdf"),
  bg = "white"
)
```
...
regression_model_coefficients_tidied_primary <-
  ...
  mutate(Model = if_else(
    row_number() <= 4,
    paste0(Model, " - acquisition model"),
    paste0(Model, " - extinction model")
  )) %>%
  group_by(Model) %>% 
  gt() %>% 
  tab_style(
...
regression_model_coefficients_tidied_secondary <-
  ...
  mutate(Model_x = case_when(
    row_number() <= 6 ~ "acquisition model",
    row_number() <= 12 ~ "extinction model",
    row_number() <= 16 ~ "acquisition model",
    row_number() <= 20 ~ "extinction model",
    row_number() <= 24 ~ "acquisition model",
    row_number() <= 28 ~ "extinction model"
  )) %>%
...
regression_model_fits_tidied <- 
  ...
  mutate(
    `Analysis type` = case_when(
      str_detect(Model, "^pri_") ~ "Primary",
      str_detect(Model, "^sec_") ~ "Secondary",
      str_detect(Model, "^expl_") ~ "Exploratory"
    ),
    model_a = case_when(
      str_detect(Model, "_glance$") ~ "Expectancy ratings",
      str_detect(Model, "^sec.*_glance1$") ~ "Age and sex adjustment",
      str_detect(Model, "^sec.*_glance2$") ~ "GAD-7 follow-up (DV)",
      str_detect(Model, "^sec.*_glance3$") ~ "FSS follow-up (DV)",
      str_detect(Model, "^expl.*_glance1$") ~ "Discrimination scores",
      str_detect(Model, "^expl.*_glance2$") ~ "Affective ratings",
      str_detect(Model, "^expl.*_glance3$") ~ "Non-FC predictors",
      str_detect(Model, "^expl.*_glance4$") ~ "Depression adjustment",
      str_detect(Model, "^expl.*_glance5$") ~ "Low GAD-7 excluded",
      str_detect(Model, "^expl.*_glance6$") ~ "Non-FC predictors"
    ),
    model_b = case_when(
      str_detect(Model, "acq_plus") ~ "acquisition CS+",
      str_detect(Model, "acq_minus") ~ "acquisition CS-",
      str_detect(Model, "ext_plus") ~ "extinction CS+",
      str_detect(Model, "ext_minus") ~ "extinction CS-",
      str_detect(Model, "acq_diff") ~ "acquisition",
      str_detect(Model, "ext_diff") ~ "extinction",
      str_detect(Model, "asi_model") ~ "ASI",
      str_detect(Model, "cbas_model") ~ "CBAS",
      str_detect(Model, "eacs_model") ~ "eACS"
    ),
    Model = paste0(model_a, " - ", model_b),
    .before = everything()
  ) %>%
  mutate(
    p = if_else(p.value < 0.001, paste0("<0.001"), paste0(round(p.value, 3))),
...
#### Converted to gt table

```{r}
regression_model_fits_tidied_gt <- 
regression_model_fits_tidied %>% 
  filter(`Analysis type` != "Exploratory") %>%
...
suppressMessages(gtsave(
  regression_model_fits_tidied_gt,
  filename = here("reports", "tables", "regression_model_fits_table.png"),
  expand = 10
))
```
...
elephant_plot_exclusions_applied <- elephant_long %>%
  ...
  geom_line() +
  geom_ribbon(
    aes(ymin = ci_low, ymax = ci_high, fill = Stimulus),
    linetype = 0,
    alpha = 0.3,
    linewidth = 0.8
  ) +
...
# Save
ggsave(
  file = here("reports", "figures", "elephant-plot_exclusions-applied.pdf"),
